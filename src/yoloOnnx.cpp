...
void YoloOnnx::SetClassThreshold(float classThreshold) {
	_classThreshold = classThreshold;
}

void YoloOnnx::SetNmsThreshold(float nmsThreshold) {
	_nmsThreshold = nmsThreshold;
}

// 加载模型
bool YoloOnnx::ReadModel(const std::string& modelPath, bool isCuda, int cudaID, bool warmUp) {
...
// 预处理
int YoloOnnx::Preprocessing(const std::vector<cv::Mat>& srcImgs, std::vector<cv::Mat>& outSrcImgs, std::vector<cv::Vec4d>& params) {
	outSrcImgs.clear();
...
bool YoloOnnx::OnnxDetect(cv::Mat& srcImg, std::vector<OutputParams>& output) {
...
bool YoloOnnx::OnnxBatchDetect(std::vector<cv::Mat>& srcImgs, std::vector<std::vector<OutputParams>>& output) {
